...
void rusiuotiStudentus(list<Student>& studentai, list<Student>& vargsiukai) {
    for (auto it = studentai.begin(); it != studentai.end(); ) {
        if (it->galutinisVid < 5.0) {
            vargsiukai.push_back(*it); // Perkeliamas „vargšiukas“ į naują sąrašą
            it = studentai.erase(it);  // Ištrinama iš pradinio „studentai“ sąrašo
        } else {
            ++it;  // Jei studentas nėra „vargšiukas“, pereiname prie kito
        }
    }
}
